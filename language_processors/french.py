...
def process_text(text):
    ...
    # Convert numbers to French words (adding spaces before and after)
    text = re.sub(r"\d+", lambda m: f" {num2words(int(m.group()), lang='fr')} ", text)

    # Standardize apostrophes
    text = re.sub(r"['＇']", "'", text)

    # Convert special characters to spaces (preserving Roman alphabets and all types of apostrophes)
